...
to move-along-circle [r]
 let distance-to-move (pi * r / 180) * (speedt / 50) + .5 * ( size + [ size ] of self  )
 ifelse not any? other turtles-on patch-ahead distance-to-move
     [fd (pi * r / 180) * (speedt / 50)
     rt speedt / 50
     set speedt  (speedt + 1)
     ]
     [ set speedt (speedt - 1) 
  ...
  ;;[fd (pi * r / 180) * (speedt / 50)
 ;; rt speedt / 50]
  ;;[fd (pi * r / 180) * (speedt / 2 * 50)
 ;; rt speedt / 2 * 50]
  ;;set heading 90 - 360 / (2 * number)
  ;;face patch-right-and-ahead (90 - 360 / (2 * number)) 1
end
...
PENS
"distance" 1.0 0 -10899396 true "" "plot [speedt] of turtle 0"
